  ...
  )
{
 ...
 if (EFI_ERROR (Status)) {
  ...
  Addr = (Address & ADDR_MASK_32BIT) | PCI_UNCACHE_ADDR_BASE;
  ...
  )
{
  ...
  Addr = (Address & ADDR_MASK_32BIT) | PCI_UNCACHE_ADDR_BASE;
  return mCpuIo->Mem.Write (mCpuIo, (EFI_CPU_IO_PROTOCOL_WIDTH) Width, Addr, Count, Buffer);
}
  ...
  )
{
  ...
  //DEBUG ((EFI_D_INFO, "begin Root Bridge Io Pci Access in loongarch64\n"));
  Status = RootBridgeIoCheckParameter (This, PciOperation, Width, Address, Count, Buffer);
  //DEBUG ((EFI_D_INFO, "in root bridge Io Pci Access in loongarch64, check params, status: %d\n", Status));
  if (EFI_ERROR (Status)) {
    DEBUG((EFI_D_INFO, "!!!!!-%d-root bridge io check parameter error\n", __LINE__));
    return Status;
  }
  ...
  //DEBUG ((EFI_D_INFO, "begin Pci segment read buffer\n"));
  //
  ...
  //DEBUG ((EFI_D_INFO, "end Pci segment read buffer\n"));
  return EFI_SUCCESS;
}
